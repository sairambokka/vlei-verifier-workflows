 ...
 */
function readFileSync(path: string, options?: string | { encoding?: string; flag?: string }): string | Buffer {
  ...
  throw new Error('Cannot synchronously read files in browser environment without prior async access');
}
...
async function readFile(
  ...
  try {
    ...
    const fileData = await new Promise<FileData | undefined>((resolve, reject) => {
      const request = store.get(path);
    ...
    try {
      localStorage.setItem(`fs-cache:${path}`, JSON.stringify({
 ...
 */
async function exists(path: string, callback?: (exists: boolean) => void): Promise<boolean | void> {
  path = normalizePath(path);
...
async function mkdir(
  ...
  try {
    if (recursive) {
      ...
      if (!parentExists) {
        throw new Error(`ENOENT: parent directory does not exist '${parentPath}'`);
      }
  ...
  try {
    ...
    for (const key of [...fileKeys, ...dirKeys]) {
      ...
      if (keyStr.startsWith(pathPrefix)) {
        ...
        const entryName = firstSlash === -1 ? remaining : remaining.slice(0, firstSlash);
...
export const promises = {
  readFile: async (path: string, options?: string | { encoding?: string; flag?: string }): Promise<string | Buffer> => {
    return readFile(path, options) as Promise<string | Buffer>;
  },
  
  writeFile: async (path: string, data: string | Buffer | Uint8Array, options?: string | { encoding?: string; mode?: number; flag?: string }): Promise<void> => {
    return writeFile(path, data, options) as Promise<void>;
  },
  
  mkdir: async (path: string, options?: { recursive?: boolean; mode?: number } | number): Promise<void> => {
    return mkdir(path, options) as Promise<void>;
  },
  
  readdir: async (path: string, options?: { encoding?: string; withFileTypes?: boolean }): Promise<string[]> => {
    return readdir(path, options) as Promise<string[]>;
  },
